...
for app_file in apps:
    ...
    # A chart pulled from a Helm repository (cert-manager) has no path: its
    # manifests are not in this tree, so there is nothing to walk. It is
    # still an app on the cluster, so it gets its node and nothing more.
    # Crashing here refused the whole k3s-iac snapshot, the same KeyError
    # iac-gate hit when the first such app landed.
    if "path" not in src:
        nodes.append({"id": f"app:{name}", "kind": "app", "label": name,
                      "layer": "cluster", "source": rel_app,
                      "meta": {"namespace": namespace,
                               "chart": f"{src.get('chart', '?')}@"
                                        f"{src.get('targetRevision', '?')}"}})
