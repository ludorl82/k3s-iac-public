...
`*.lab.example` comes from **Let's Encrypt** since 2026-09, obtained and renewed by cert-manager (`argocd/apps/cert-manager.yaml`, Helm chart pinned by hand, not by Renovate) with a DNS-01 challenge on the `lab.example` Cloudflare zone. `tls/` holds the two ClusterIssuers (staging, prod) and the `Certificate`, which writes the secret `kube-system/wildcard-lab-example-le`.

Why it replaced the private-CA wildcard: that one was valid for ten years, and **macOS refuses any TLS server certificate valid for more than 825 days**, even under a trusted root. Every Apple device rejected the `*.lab.example` services. `lab.example` is a real domain delegated to Cloudflare with an empty public zone (every name is a pfSense Unbound override), so DNS-01 works without any service name becoming public. Only `*.lab.example` appears in Certificate Transparency logs.

Two things to know:

- **cert-manager checks propagation against public resolvers** (`--dns01-recursive-nameservers=1.1.1.1:53,8.8.8.8:53`). The cluster resolver ends at pfSense's Unbound, which answers `lab.example` itself and never sees the `_acme-challenge` record at Cloudflare.
- **Always try staging first.** A broken setup against production burns Let's Encrypt's failed-validation quota in minutes.

Secrets expected in the `cert-manager` namespace (a ClusterIssuer reads from there):
- `cloudflare-api-token` (`api-token`) — a token **dedicated** to cert-manager, never reused: Zone → DNS → Edit and Zone → Zone → Read, on the `lab.example` zone only. Named **`wildcard-lab-example-cert`** in the Cloudflare dashboard, stored in KeePass as **"Cloudflare Cert Renewal 2"**. Verified 2026-09-23: active, sees `lab.example`, does **not** see `pub.example.com`. Create the secret without the value ever touching the process list or disk, and strip the newline:

  ```
  ssh console-vm 'kp-get "Cloudflare Cert Renewal 2"' | tr -d '\n' \
    | kubectl -n cert-manager create secret generic cloudflare-api-token --from-file=api-token=/dev/stdin
  ```

  Check a token before trusting it: `.../client/v4/user/tokens/verify` must answer `active`. A first attempt read a stale KeePass entry that shared the title, and Cloudflare answered `1000 Invalid API Token`: **`kp-get` silently picks one entry when two share a title.**
...
## renovate.json + iac-gate/ — nobody merges

Since 2026-09-16 no image bump in this repo is merged by a person. The decision was taken after a month in which the single grouped Renovate PR (#23) sat conflicting and unmerged while seven images drifted: a merge step that needs a human is a merge step that does not happen.

**Renovate** (`renovate.json`, run daily by `scripts/renovate-run.sh` from console-vm via the Cronicle "Daily IaC updates" event) opens **one PR per image** and **automerges it itself** once the tier's upstream release age has passed: patch/digest immediately, minors after 3 days, majors after 14, and the fragile list (Frigate, Loki, Argo CD, mongo) after 14/30. `ignoreTests: true` is deliberate — this repo has no CI, and the test is what happens *after* the merge. Renovate automerges at most one PR per run and never the one it just opened, which is why the runner loops and why the job is daily. Only the mongo major stays disabled (FCV runbook).

**iac-gate** (`iac-gate/`, a CronJob every 10 minutes in its own namespace, rendered by kustomize so `gate.py` is a real file) is the test. It clones main, takes the Renovate commits of the last 6 hours that no gate trailer has answered, and 15 minutes after each one asks: did Argo CD deploy it (`status.sync.revision` contains the commit), is every app it touched healthy (not Degraded/Missing/still Progressing, last sync not Error/Failed), and are the app's Kuma monitors green on a beat newer than the commit? The Kuma monitors come from `apps.json` and are read off the public **`gate` status page** in Kuma (in-cluster, no credentials) — add a monitor there when you add it to `apps.json`. Apps not in `apps.json` are judged on Argo health alone.

A red answer reverts the commit on main (deploy key `iac-gate`, write access — the only writer besides Renovate), appends an `iac-gate auto-block until <date>` packageRule to `renovate.json` so the same tag is not re-proposed for 30 days, and pages ntfy. A revert that conflicts is recorded with an `Iac-Gate-Skip` trailer and paged as urgent instead of being retried every 10 minutes. A commit Argo has not picked up after 30 minutes is paged once, not reverted. The gate expires its own blocks. Every run ends with a Kuma push (monitor "iac-gate", 30 min) — a gate that stops running is itself an alert.

**Nobody gets paged for a rollout either.** The first Renovate pass runs with automerge forced off, so every open Renovate PR is one that will merge in the next pass; `scripts/renovate-run.sh` maps their files to Argo apps, `iac-gate/silence.json` maps apps to the Kuma monitors that would page (wider than `apps.json`: it includes the push monitors of host-side jobs that talk to the service), and `scripts/kuma-silence.py` opens a timed Kuma maintenance window on them before the merge pass. The gate ignores a monitor under maintenance; when the window ends Kuma pages for real if the service is still down. Added after Frigate 0.18.0 paged twice during its 10 GB pull on 2026-09-16.

Secrets in the `iac-gate` namespace, created out-of-band: `iac-gate-deploy-key` (`id_ed25519`), `iac-gate-ntfy` (`token`, ntfy user `iac-gate`, write-only to `alerts`), `iac-gate-kuma` (`push-url`).

What this does not cover: the six images pinned to `:latest` (ntfy, cloudflared, n8n, unifi, the GitHub runner, cronicle) float outside Renovate and upgrade whenever their pod restarts. Pin them to get them under the gate.
